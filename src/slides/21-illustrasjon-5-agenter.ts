...
const slide: Slide = {
    ...
    <section class="slide">
      ...
      <div class="diagram">
        <svg viewBox="0 0 1000 480" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="NFK chatbot: orkestrator + 5 fagagenter">
          ...
          <g class="ovc-node">
            <circle class="orch-pulse" cx="420" cy="240" r="80" fill="url(#ovc-grad)" filter="url(#ovc-soft)" opacity="0.55"/>
          ...
          <!-- Paths (visible) -->
          <path id="path-innbygger" d="M 200 240 C 280 240, 320 240, 352 240" stroke="rgba(255,255,255,0.4)" stroke-width="2" fill="none"/>
          <path id="path-agent-0" d="M 488 210 C 580 170, 660 90, 760 65" stroke="#F86233" stroke-width="2" fill="none" opacity="0.7"/>
          <path id="path-agent-1" d="M 488 225 C 580 200, 660 165, 760 155" stroke="#DA446E" stroke-width="2" fill="none" opacity="0.7"/>
          <path id="path-agent-2" d="M 488 240 C 580 240, 660 240, 760 245" stroke="#BC25A9" stroke-width="2" fill="none" opacity="0.7"/>
          <path id="path-agent-3" d="M 488 255 C 580 285, 660 320, 760 335" stroke="#861FCB" stroke-width="2" fill="none" opacity="0.7"/>
          <path id="path-agent-4" d="M 488 270 C 580 320, 660 400, 760 425" stroke="#521CE4" stroke-width="2" fill="none" opacity="0.7"/>
          ...
          <circle id="msg-dot" cx="120" cy="240" r="7" fill="#F86233" filter="url(#msg-glow)" opacity="0"/>
        </svg>
      </div>
  ...
  onMount(root) {
    ...
    // Melding-animasjon: innbygger → orkestrator → tilfeldig agent, loop
    const dot = root.querySelector<SVGCircleElement>('#msg-dot');
    const pathIn = root.querySelector<SVGPathElement>('#path-innbygger');
    const agentPaths = [0, 1, 2, 3, 4].map((i) =>
      root.querySelector<SVGPathElement>(`#path-agent-${i}`)
    );
    if (dot && pathIn && agentPaths.every(Boolean)) {
      const stop = loop(
        [
          async (s) => {
            ...
            await animateAlongPath(dot, pathIn, 700, s);
            await wait(280, s);
